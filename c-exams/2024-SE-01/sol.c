...
int main(int argc, char* argv[]) {
    ...
    while((read_bytes = read(fd, &c_node, sizeof(c_node))) > 0) {
        ...
        if(c_node.next * sizeof(struct) >= f.st_size) { warnx(8, "referring to non-existing byte, check format"); break; }
        if(lseek(fd, c_node.next * sizeof(struct), SEEK_SET) < 0) { err(7, "cannot lseek"); }
        curr = c_node.next;
    }
